...
def solve_almost_tridiagonal_system(const double diag_element, const double sub_diag, const double up_diag,
									const int size, possible_type[::1] current_state, possible_type[::1] u):
	cdef possible_type denom = 1
	cdef possible_type factor = 0
	cdef double first_diag = 2 * diag_element
	cdef double last_diag = diag_element + sub_diag * up_diag / diag_element
	cdef Py_ssize_t i = 0

	u[0] = <possible_type> -diag_element
	u[size - 1] = <possible_type> up_diag
	...
	denom = <possible_type>(1 + u[0] - sub_diag * u[size - 1] / diag_element)
	factor = <possible_type>(current_state[0] - sub_diag * current_state[size - 1] / diag_element) / denom
...
cdef void ThomasAlgorithm(const double sub_diag_element, const double diag_element,
						  const double super_diag_element, possible_type[::1] d, const int size,
						  const double first_diag_element, const double last_diag_element):
	cdef Py_ssize_t i
	cdef possible_type* bprimes

	bprimes = <possible_type*> malloc(sizeof(possible_type) * size)
	bprimes[0] = <possible_type> first_diag_element
	for i in range(1, size - 1):
		bprimes[i] = <possible_type> (diag_element - sub_diag_element * super_diag_element / bprimes[i - 1])
		d[i] -= <possible_type> (sub_diag_element * d[i - 1] / bprimes[i - 1])

	bprimes[size - 1] = <possible_type> (last_diag_element - sub_diag_element * super_diag_element / bprimes[size - 2])
	d[size - 1] -= <possible_type> (sub_diag_element * d[size - 2] / bprimes[size - 2])

	d[size - 1] /= bprimes[size - 1]
	for i in range(size - 2, -1, -1):
		d[i] -= <possible_type> super_diag_element * d[i + 1]
		d[i] /= bprimes[i]
